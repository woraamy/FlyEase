...
import { getTravelPlans } from "../action";

export default async function TravelPlansPage() {
    let initialTravelPlans = [];
      try {
    ...
    <main className="min-h-screen bg-white">
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {initialTravelPlans.map((plan: { _id: string; header_topic: string; departure_country: string; arrival_city: string; arrival_country: string; introduction: string; header_img: string; }) => (
            <TravelPlanCard
                key={plan._id}
